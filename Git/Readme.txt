...
2. `cd ..`
3. `cd /mnt/c`
4. `cd ~` - Vamos a la raíz
5. `mkdir Tecnicatura` - Recordar que en Windows las mayúsculas no tienen relevancia, pero sí en Linux
6. `cd tecnicatura`
7. `mkdir Python`
8. `mkdir Java`
9. `mkdir JavaScript`
...
- `touch vacio.txt` - Crea un archivo con su extensión
  - **ESCRIBIR DENTRO**
  ...
  - `rm --help` - Muestra cómo funciona el comando

### Crear un Repositorio de Git y Hacer el Primer Commit

1. `cd tecnicatura`
2. `mkdir class-git`
3. `cd class-git` - Entramos en la carpeta que necesitamos trabajar
4. `git init` - Creamos un repositorio en la carpeta central, se crea el archivo `.git`
5. `code .` - Abrimos VSC, el punto hace que se abra el archivo en el que estamos situados
6. `ctrl + n` - Creamos un archivo nuevo y escribimos en él, como lo hicimos antes
7. `ctrl + s` - Guardamos poniendo el nombre: `historia.txt`
8. `git status` - Vemos el estado del proyecto en tiempo real, está en el área de trabajo
9. `git add historia.txt` - Enviamos el archivo al área de preparación
10. `git status` - Para ver el estado de cambios
11. `git rm --cached historia.txt` - Quitamos el archivo del área de preparación, cached significa que está en memoria RAM
12. `git config` - Tendremos la lista de cómo funciona la configuración
13. `git config --list` - Configuraciones por defecto, faltan cosas importantes
14. `git config --list --show-origin` - Veremos dónde están las configuraciones guardadas
15. `git config --global user.name "Ariel Betancud"`
16. `git config --global user.email "betancudariel@gmail.com"` - El correo debe ser el mismo que usaremos en GitHub
17. `git config --list` - Ahora veremos que ya están todos los datos completos
18. `git add .` - Ingresamos todos los archivos al área de preparación (RAM)
19. `git commit -m "Mensaje importante del commit"` - El primer commit está hecho
20. `code .` - Hacemos cambios en el archivo y guardamos
21. `git status` - Hay cambios para commitear
22. `git add .`
23. `git commit -m "Mi segundo commit"`
24. `git log historia.txt` - Vemos toda la historia de este archivo, el número largo es el hash del commit
...
1. `cd tecnicatura` - Ingresamos al directorio donde están nuestras carpetas de trabajo
2. `ls` - Vemos los archivos y directorios que ya tenemos
3. `cd git` - No hay nada
4. `cd ..` - Salimos
5. `rm historia.txt` - Eliminamos el archivo que habíamos hecho, esto en git bash (Windows) esto es para práctica
6. `rm Git` - `rm: cannot remove 'Git': Is a directory`
7. `rm --recursive -R Git` - By default, rm does not remove directories. Use the --recursive (-r or -R) arguments option to remove each listed directory, too, along with all of its contents. Esto es para práctica
8. `rm --help` - Nos muestra lo que les puse arriba como documentación en Inglés.
9. `mkdir class-git` - Creamos la carpeta o directorio para trabajar en Git local por ahora.
10. `cd class-git` - Entramos para crear el `README.md` para este sector.
11. `touch README.md` - Vamos a crear un archivo nuevo, `md` significa markdown y se pueden trabajar con editores de texto, este es un lenguaje que transforma el texto a html.
...
12. `code .` - Abrimos VSC para editar el archivo.
13. Empezamos a cargar lo visto en las clases anteriores (Comandos) en el `README` y pasamos a commitear
14. `git status`
15. `git add .`
16. `git status`
17. `git commit -m "Cargamos el README dentro del directorio class-git"`
18. `git status`
19. `git log` - Para ver los dos commits hechos: Si tienes commiteada alguna clase anterior veras más commits de los que yo tengo.
20. `cd ..`
21. `cd ..`
...
**Tarea -> Agregar los comentarios en los comandos, para saber qué pasa con cada uno.**

1. `cd tecnicatura`
2. `cd class-git`
3. `ls`
4. `touch historia.txt`
5. `code .`
...
Conflictos en repositorios remotos Al trabajar con otras personas, es necesario utilizar un repositorio remoto. ­ -Para copiar el repositorio remoto al directorio de trabajo local, se utiliza el comando git clone , y para enviar cambios al repositorio remoto se utiliza git push.
...
Los merges pueden generar conflictos, esto aborta la acción y pide que soluciones el problema manualmente, aceptando o rechazando los cambios que vienen.<s/ub>
